...
#
# for_next for production run
#
# Usage: for_next.py -c md9 -n md10
#
...
if list(p.glob('**/*.pdd'))  and not args.histogram:
    ...
    for s in tqdm(sorted(p.glob("**/md.vmcmd.ene")),desc='Ene(Pass2)'):
        ...
        for i in range(nvst):
            ...
            # y, ind = np.histogram(ene3,bins=bin_index)
...
#  rm md.vmcmd.ene
  time=$((($SLURM_ARRAY_TASK_ID -1) * shift))
  echo "$time" | gmx eneconv -quiet -settime yes -b $((time + 2)) -f md.edr -o md_nt.edr
  echo "1" | gmx trjconv -quiet -s md.tpr -f md.xtc -o md_noSolpbc.xtc -pbc mol -b 2 -t0 $time
  echo "1" | gmx trjconv -quiet -s ../../em1/em1.gro -f md_noSolpbc.xtc -b $time -e $time -o md_Ref.gro
  echo -e "3\n1" | gmx trjconv -quiet -s md_Ref.gro -f md_noSolpbc.xtc -o md_fit.xtc -fit "rot+trans"
fi
...
constraints         = h-bonds
;
'''.format(NSTEPS=10000000, NSTENERGY=1000, NSTXOUT=1000))
